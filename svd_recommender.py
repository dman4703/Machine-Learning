...
class SVDRecommender(object):
    ...
    def recommender_svd(self, R: np.ndarray, k: int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        sqrtSk = np.sqrt(S[:kActual])
        Uk = U[:, :kActual] * sqrtSk
        Vk = (sqrtSk[:, None]) * Vt[:kActual, :]
        return Uk, Vk
    # end recommender_svd
    ...
    def predict(
        ...
        """
        rHat = np.dot(U_k, V_k)
        userIndex = users_index[user_id]

        recommendations = []
        scores = []
        for movie in movies_pool:
            movieID = self.get_movie_id_by_name(movie)
            if movieID not in movies_index:
                continue
            # end if
            movieIndex = movies_index[movieID]
            if not np.isnan(R[userIndex, movieIndex]):
                continue
            # end if
            score = rHat[userIndex, movieIndex]
        ...
        topIndices = np.argsort(scores)[::-1][:top_n]
        topMovies = []
        for i in topIndices:
            topMovies.append(recommendations[i])
        # end for
        return np.array(topMovies)
    # end predict
