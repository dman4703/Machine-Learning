...
def get_mri_dataset(classes):
    ...
    if all(os.path.exists(path) for path in save_paths.values()):
        ...
        x_train = torch.load(save_paths["x_train"], weights_only=False)
        y_train = torch.load(save_paths["y_train"], weights_only=False)
        x_test = torch.load(save_paths["x_test"], weights_only=False)
        y_test = torch.load(save_paths["y_test"], weights_only=False)
        return x_train, y_train, x_test, y_test
