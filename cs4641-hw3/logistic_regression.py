...
class LogisticRegression(object):
    ...
    def predict_labels(self, h_x: np.ndarray, thresold: float) -> np.ndarray:
        ...
        """
        return (h_x >= thresold).astype(np.int64)
    # end predict_labels
    ...
    def gradient(self, x_aug: np.ndarray, y: np.ndarray, h_x: np.ndarray) -> np.ndarray:
        ...
        """
        numSamples = x_aug.shape[0]
        grad = np.dot(x_aug.T, h_x - y) / numSamples
        return grad
    # end gradient
    ...
    def accuracy(self, y: np.ndarray, y_hat: np.ndarray) -> float:
        ...
        """
        correctPredictions = (y_hat == y).astype(np.float64)
        return float(np.mean(correctPredictions))
    # end accuracy

    def evaluate(
        self, x: np.ndarray, y: np.ndarray, theta: np.ndarray, threshold: float
    ) -> Tuple[float, float]:
        """
        ...
        """
        xAug = self.bias_augment(x)
        probPredictions = self.predict_probs(xAug, theta)
        loss = self.loss(y, probPredictions)
        labelPredictions = self.predict_labels(probPredictions, threshold)
        accuracy = self.accuracy(y, labelPredictions)
    ...
    def fit(
        ...
        xTrainAug = self.bias_augment(x_train)

        for epoch in range(epochs):
            hX = self.predict_probs(xTrainAug, theta)
            grad = self.gradient(xTrainAug, y_train, hX)
            theta -= lr * grad
...
def hyperparameter_tuning(
    ...
    for currentThreshold in thresholds:
        _, currentAccuracy = model.evaluate(
            x_test, y_test, theta, float(currentThreshold)
        )

        if bestThreshold is None:
            bestThreshold = float(currentThreshold)
        ...
        if currentAccuracy > bestAccuracy:
            bestThreshold = float(currentThreshold)
            bestAccuracy = currentAccuracy
        elif np.isclose(currentAccuracy, bestAccuracy) and (float(currentThreshold) < bestThreshold):
            bestThreshold = float(currentThreshold)
            bestAccuracy = currentAccuracy
        #end if
    # end for
    return bestThreshold, bestAccuracy
# end hyperparameter_tuning